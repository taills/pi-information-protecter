...
export interface MigrationOptions {
  ...
  checkpoint?: (
    stage: "beforeCommit" | "afterCommit" | "beforeDelete",
    path: string,
  ) => void;
}
...
export function validatePatterns(configs: Config[]): Promise<void> {
  return new Promise((resolvePromise, reject) => {
    const worker = new Worker(
      fileURLToPath(new URL("./validate-worker.mjs", import.meta.url)),
      {
        workerData: configs,
        execArgv: [],
...
function names(dir: string): string[] {
  return fs
    .readdirSync(dir)
    .filter((name) => CONFIG_NAME.test(name))
    .sort();
}
...
export async function migrateConfig(
  ...
  const lock = join(dir, ".protecter-migration.lock"),
    deadline = Date.now() + (options.lockTimeoutMs ?? 5000);
  for (;;) {
    try {
  ...
  try {
    ...
    if (temp) {
      try {
        ...
        /* Preserve on failure. / 失败时保留待人工检查。 */
      }
    }
    fs.rmdirSync(lock);
  }
