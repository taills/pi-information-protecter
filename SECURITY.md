...
- The model reasons about the replacement, so any answer derived from a protected value, such as a digit position, a checksum, a comparison or arithmetic, is wrong. Only the value is restored in the reply, never the conclusion drawn from it, so the reply looks confident and stays consistent across retries. Verify such answers locally.
  模型推理的是替换值，因此任何从受保护值推导出的答案（位数、校验位、比较、算术）都是错的。回复中只有值被还原，由它推导出的结论不会，因此答案看似笃定且重试结果一致。请在本地核对。
...
- Compaction summaries are generated by this extension from redacted text, because Pi builds the summarization request internally and passes no `onPayload` callback, so `before_provider_request` never fires for it. Letting Pi summarize would send the stored conversation, which holds restored plaintext, to the provider in full. Any failure cancels compaction instead of falling back to that path. The summary still reaches the provider as redacted text, and the model reasons about replacements, so it may describe protected values inaccurately.
  压缩摘要由本扩展基于脱敏文本生成，因为 Pi 在内部构造摘要请求且不传递 `onPayload` 回调，`before_provider_request` 对其不会触发。直接交给 Pi 生成摘要会把存有还原明文的会话内容完整发给提供商。任何失败均取消压缩，而不回退到该路径。摘要仍以脱敏文本到达提供商，且模型推理的是替换值，对受保护值的描述可能不准确。
- Summarized `/tree` navigation takes the same internal path and is produced the same way, from redacted entries. Do not widen Pi compatibility without verification: refusing a working release blocks every request, and accepting a broken one can send plaintext. Each supported version is established by running the full suite against it, and the runtime gate and declared peer range are kept identical.
  带摘要的 `/tree` 导航走同一条内部路径，同样基于脱敏条目生成。未经验证不扩大 Pi 兼容范围：误拒可用版本会拦截所有请求，误受损坏版本则可能发送明文。每个受支持版本均以完整测试验证，并保持运行时门禁与声明的 peer 范围一致。
